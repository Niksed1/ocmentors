...
export default function TutorsPage({ params }: { params: Promise<{ subject: string }> }) {
  const { subject } = use(params);
  const tutors = tutorsData as Tutor[];
  const subjects = subjectsData as Subject[];
  
  const subjectTutors = tutors.filter(tutor => tutor.subject === subject);
          ...
          <div className="mb-12">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <h1 className="text-4xl sm:text-5xl font-bold text-white">
                {currentSubject?.name || subject.charAt(0).toUpperCase() + subject.slice(1)} Tutors
              </h1>
            </div>
            <p className="text-xl text-white/70">
              Choose from {subjectTutors.length} experienced tutors
            </p>
          </div>
              ...
              {subjectTutors.map((tutor) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/20">
                    ...
                    <button className="px-6 py-2 bg-primary-green text-white rounded-lg hover:bg-accent-green transition-colors font-medium">
                      Book Session
                    </button>
                  </div>
